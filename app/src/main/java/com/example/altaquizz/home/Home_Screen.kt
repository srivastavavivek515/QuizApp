...
fun HomeScreen(state:StateHomeScreen,navController:NavHostController,event:(EventHomeScreen)->Unit) {
    Column(Modifier.verticalScroll(rememberScrollState())) {
        ...
        Column(modifier = Modifier.padding(20.dp),horizontalAlignment = Alignment.CenterHorizontally) {
            AdvancedDropdownMenu("No Of Question",Constants.noOfQuestion,state.noOfQuestion.toString(), onDropDownChange = {
             event(EventHomeScreen.SetNoOfQuestion(data = it))
            })
            AdvancedDropdownMenu("Select Category",Constants.category,state.category, onDropDownChange = {
             event(EventHomeScreen.SetCategory(data = it))
            })
            AdvancedDropdownMenu("Difficulty",Constants.difficulty,state.difficulty, onDropDownChange = {
            ...
            RoundedCornerButton("Start Exam", colorResource(id = R.color.black),Color.White){
    ...
    Box(modifier = Modifier
        ...
        Row(modifier = Modifier.fillMaxSize(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) {
            ...
            Icon(modifier = Modifier
                .width(40.dp)
                .height(40.dp), painter = painterResource(id = R.drawable.menu_open), contentDescription = "menu_button", tint = colorResource(
                id = R.color.iconColor
            ))
            Text(text = "Quiz", fontWeight = FontWeight.Bold, fontSize = 35.sp, color = colorResource(
                id = R.color.iconColor
            ))
            Icon(modifier = Modifier
                .width(40.dp)
                .height(40.dp),painter = painterResource(id = R.drawable.contact_mail), contentDescription = "contact_button", tint = colorResource(
                id = R.color.iconColor
            ))
        }
    }
